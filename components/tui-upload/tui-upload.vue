	...
	<view class="tui-container">
		<view class="tui-upload-box">
			<view class="tui-image-item" v-for="(item,index) in imageList" :key="index">
				<image :src="item" class="tui-item-img?imageView2/0/w/140" @tap.stop="previewImage(index)" mode="aspectFill"></image>
				<!-- <view v-if="!forbidDel" class="tui-img-del" @tap.stop="delImage(index)"></view> -->
				<image src="/static/upload/delete.png" v-if="!forbidDel" class="tui-img-del" @tap.stop="delImage(index)"></image>
			...
			<view v-if="isShowAdd" class="tui-upload-add" @tap="chooseImage">
				...
				<image src="/static/upload/add.png"></image>
			</view>
		</view>
	</view>
</template>
						...
						success: function(res) {
							...
							if (res.statusCode == 200) {
								...
								if (d.code % 100 === 0) {
									...
									d.data.src && (_this.imageList[index] = d.data.src)
									_this.$set(_this.statusArr, index, d.data.src ? "1" : "3")
								} else {
									// 上传失败
